...
async def _build_daily_report(db: AsyncSession) -> dict:
    ...
    summary = {
        ...
        "actions_24h": int(await db.scalar(select(func.count(AuditLog.id)).where(AuditLog.timestamp >= since)) or 0),
    }
...
async def download_daily_report(
    ...
    """Download a daily SOC report as a JSON file."""
    payload = await _build_daily_report(db)
    filename = f"defmon_daily_report_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.json"
